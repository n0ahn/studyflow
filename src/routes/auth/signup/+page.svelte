...
<div class="relative min-h-screen flex flex-col justify-between bg-background text-foreground overflow-hidden px-6 py-8">
    ...
    <!-- Registreerformulier gecentreerd -->
    <main class="relative z-10 flex-1 flex items-center justify-center">
        <div class="w-full max-w-md card p-8 backdrop-blur-xl bg-card/80 border-border shadow-2xl flex flex-col gap-6">
            <div>
                <h1 class="text-2xl font-bold tracking-tight mb-1">Account aanmaken</h1>
                <p class="text-sm text-muted-foreground">Start vandaag nog met slim studeren en haal alles uit je toetsen.</p>
            </div>

            {#if success}
                <div class="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm flex items-center gap-2.5">
                    <span>✨</span>
                    <p class="m-0">Check je e-mail om je account te bevestigen.</p>
                </div>
            {:else}
                {#if error}
                ...
                <form onsubmit={(e) => { e.preventDefault(); handleSignup(); }} class="flex flex-col gap-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                        <label for="confirm" class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Bevestig wachtwoord</label>
                        <input
                            id="confirm"
                            type="password"
                            placeholder="••••••••"
            ...
            <div class="text-center text-sm text-muted-foreground pt-2 border-t border-border">
                Al een account? 
                <a href={resolve('/auth/login')} class="font-medium hover:underline" style="color: oklch(from var(--accent) calc(l * 1.15) c h);">
                    Log hier in
                </a>
            </div>
        </div>
    ...
    <div class="peek-container absolute bottom-0 right-2 md:right-10 z-20 group cursor-pointer flex flex-col items-end pointer-events-none">
        ...
        >
            <p class="m-0 flex items-center gap-1.5 whitespace-nowrap">
                🚀 <span style="color: oklch(from var(--accent) calc(l * 1.15) c h); font-weight: 600;">Bijna klaar!</span> Maak je gratis account aan.
            </p>
        </div>
